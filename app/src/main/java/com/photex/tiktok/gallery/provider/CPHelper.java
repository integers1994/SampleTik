...
import com.photex.tiktok.gallery.filter.FoldersFileFilter;
import com.photex.tiktok.gallery.filter.ImageFileFilter;
...
/*
    public static Observable<Album> getAlbums(Context context,
        ...
        return hidden ? getHiddenAlbums(context, excluded) :
                getAlbums(context, excluded, sortingMode, sortingOrder);
    }
*/
    ...
    private static Observable<Album> getAlbums(Context context,
        ...
        return QueryUtils.query(query.build(),
                context.getContentResolver(), Album::new);
    }
...
/*
    private static Observable<Album> getHiddenAlbums(Context context,
        ...
        boolean includeVideo = LocalPrefManger.isIncludeVideo(context);
        return Observable.create(subscriber -> {
            try {

                ArrayList<String> lastHidden = LocalPrefManger.getHiddenFolder(context);
...
*/
    ...
    public static Observable<Media> getMedia(Context context, Album album) {
        ...
        if (album.getId() == -1) return getMediaFromStorage(context, album);
            ...
            return getMediaFromMediaStore(context,
                    album, album.settings.getSortingMode(),
    ...
    private static Observable<Media> getAllMediaFromMediaStore(Context context
        ...
        return QueryUtils.query(query.build(),
                context.getContentResolver(), new Media());
    }



    public static Observable<Media> getAllVideosFromStorage(Context context,Album album) {
//        album.settings.getSortingMode(), album.settings.getSortingOrder()
        Query.Builder query = new Query.Builder()
                .uri(MediaStore.Files.getContentUri("external"))
                .projection(Media.getProjectionVideo())//Video Projections
                .sort(album.settings.getSortingMode().getMediaColumn())
                .ascending(album.settings.getSortingOrder().isAscending());
...
*/
        ...
        return QueryUtils.queryContext(query.build(),
                context.getContentResolver(), new Media(),context);
    }
    ...
    private static Observable<Media> getMediaFromStorage(Context context, Album album) {
        ...
        return Observable.create(subscriber -> {
            ...
            File[] files =
                    dir.listFiles(new ImageFileFilter(true/*LocalPrefManger.isIncludeVideo(context)*/));
            try {
                if (files != null && files.length > 0)
                    for (File file : files)
